 ...
 * Implements the "_Promises/A+" specification.
 */

// I change this name to _Promise,because it cover original Promise
var _Promise = function() {
    ...
    this._then_Promises = []; /* promises returned by then() */
}
...
_Promise.prototype.then = function(onFulfilled, onRejected) {
    ...
    var then_Promise = new _Promise();

    this._then_Promises.push(then_Promise);
 ...
 * @param {any} value
 */
_Promise.prototype.reject = function(value) {
    if (this._state != 0) { return this; }
 ...
 */
_Promise.prototype.chain = function(promise) {
...
_Promise.prototype._processQueue = function() {
    while (this._then_Promises.length) {
...
_Promise.prototype._executeCallback = function(cb) {
    ...
    try {
        ...
        if (returned && typeof(returned.then) == "function") {
            /* 3.2.6.3. If either onFulfilled or onRejected returns a promise (call it returned_Promise), promise2 must assume the state of returned_Promise */
            var fulfillThen_Promise = function(value) { then_Promise.fulfill(value); }
            var rejectThen_Promise = function(value) { then_Promise.reject(value); }
            returned.then(fulfillThen_Promise, rejectThen_Promise);
        } else {
 ...
 */
_Promise.all = _Promise.when = function(all) {
...
/**
 * _Promise-based version of setTimeout
 */
_Promise.setTimeout = function(ms) {
    var promise = new this();
...
/**
 * _Promise-based version of addEventListener
 */
_Promise.event = function(element, event, capture) {
    var promise = new this();
    var cb = function(e) {
...
/**
 * _Promise-based wait for CSS transition end
 */
_Promise.transition = function(element) {
...
/**
 * _Promise-based version of XMLHttpRequest::send
 */
_Promise.send = function(xhr, data) {
    var promise = new this();
...
_Promise.worker = function(url, message) {
    var promise = new this();
    var worker = new Worker(url);
    _Promise.event(worker, "message").then(function(e) {
        promise.fulfill(e.data);
    });
    _Promise.event(worker, "error").then(function(e) {
...
// MM.Item.COLOR = "#999";
MM.Item.COLOR = myChange.borderColor;
...
MM.Map = function(options) {
    var o = {
        ...
        // change initial mapping format to right
...
MM.UI.Backend.WebDAV.save = function() {
    ...
    if (url.match(/\.mymind$/)) { /* complete file name */ } else { /* just a path */
...
MM.Mouse._visualizeDragState = function(state) {
    ...
    if (state) { /* show new vis */
        ...
        node.style.boxShadow = (x * offset) + "px " + (y * offset) + "px 2px " + spread + "px " + myChange.boxDragShadow;
    }
}
/*
